...
class fingerpose_csv:
    ...
    def internal_get_joint(self, st, frame_idx, joint_idx):
        try:
            ...
            print(
                f"reeee!!!! Tried to get idx {frame_idx} but file is only {len(self.file)} long")
        root = 1 + (joint_idx * 7)
    ...
    def get_hand_size(self, st, frame_idx):
        ...
        middleprox = self.internal_get_joint(
            st, frame_idx, XRT_HAND_JOINT_MIDDLE_PROXIMAL)
    ...
    def get_joint(self, st, frame_idx, joint_idx):
        ...
        try:
            ...
            print(
                f"reeee!!!! Tried to get idx {frame_idx} but file is only {len(s.file)} long")
