...
def display_and_log_output(
    ...
    canvas[:, 0:128] = make_visualization_column(
        input_image_with_maybe_predicted, ground_truth, (0, 255, 0))
    canvas[:, 128:256] = make_visualization_column(
        input_image, model, (255, 0, 0))
    ...
    if header.env_settings.gui_enabled:
        ...
        key = cv2.waitKey(1)
        # Exiting cleanly is important; sometimes you can accidentally ctrl+c
        # when saving the state dict
        if (key == ord('q')):
            exit(0)
    ...
    wandb.log({name: wandb.Image(geo.mat_float32touint8(canvas))})
