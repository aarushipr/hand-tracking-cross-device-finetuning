...
def maybe_add_image_edge_simulation(img: np.ndarray) -> np.ndarray:
    ...
    # cover part of the image in black, either a hard axial line or a circular
    # gradient
...
class AugmentationMaker:
    def __init__(self, inp_aug_config: aug_config):
        ...
        # The hardcoded path doesn't exist here and its contents were never read anyway.
        # An empty list is behaviourally identical; get_rotate_scale_background falls back.
    ...
    def make_heatmap_output(self,
        ...
        # Because PyTorch's default collate function dies if you try to use
        # dataclasses.
        sample = {
            ...
            # note: in /3/epics/grayscale-keypoint-2 is where this came from,
            # and there's more useful stuff there.
    ...
    def center_and_radius_prediction(self, gt_keypoints, prediction):
        ...
        for keypoint in gt_keypoints:
            radius_around_gt_and_predicted = max(
                radius_around_gt_and_predicted, np.linalg.norm(
                    keypoint - center))
    ...
    def warp_matrix_validation(
            self,
            keypoints,
            is_right,
            pose_predicted_keypoints=None):
        ...
        # Use our distribution code to pick a radius relative either ground
        # truth or predicted keypoints
        mul = 1.5
        final_r = r_calculate_max * mul

        # If we had a prediction and ground truth, the final_r might crop out
        # some of the ground truth keypoints. If so, make it bigger.
    ...
    def get_rotate_scale_background(self):
        if not self.backgrounds_list:
            # No indoor backgrounds configured, so fall back to zeros not random.choice([]).
            # Compositing adds no diversity until indoor_backgrounds_path is wired up.
            return np.zeros(
        ...
        # Crop at the center to sz - ensures no black edges - also, randomly
        # flip
        ...
        # Convert the background to linear as well. I don't _quite_ know if
        # they come as sRGB, but it's a good guess and doesn't matter too much.
    ...
    def do_one_augmentation(self,
        ...
        # XXX: This won't work if you use greenscreen data as a validation
        # dataset. So don't.
        if mask is not None:
        ...
        if not prediction_found:
            predicted_px = np.zeros((21, 2))

        if not has_depth:
            # THIS DOES NOT USUALLY RUN, OR LIKE EVER.
        ...
        if prediction_found:
            for i in range(21):
                predicted_px[i, :2] = geo.map_ranges(
                    predicted_px[i, :2], 0, 128, -1, 1)
                # Let's leave the third element. So it'll roughly be in -1.5,
                # 1.5.
